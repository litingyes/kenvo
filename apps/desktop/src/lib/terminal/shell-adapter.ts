...
import { ElectronFs } from '@/lib/terminal/electron-fs'
...
export class ShellAdapter {
  ...
  constructor(opts: {
    ...
    this.electronFs = new ElectronFs()
  }

  async init(): Promise<void> {
    this.homeDir = await api.path.homeDir().catch(() => '/')

    const baseFs = new InMemoryFs()
    const electronFs = new ElectronFs(this.homeDir)
    const mountable = new MountableFs({ base: baseFs })
    mountable.mount(this.homeDir, electronFs)
  ...
  private async trackCwd(cmd: string): Promise<void> {
    ...
    try {
      const info = await this.electronFs.stat(resolved)
      if (info.isDirectory) {
  ...
  private async tabComplete(): Promise<void> {
    ...
    try {
      const entries = await this.electronFs.readdir(dir)
    ...
    if (candidates.length === 1) {
      ...
      if (completion) {
        ...
        try {
          ...
          const info = await this.electronFs.stat(fullPath)
